...
export default function RemoveEventAttendeeModal({
    eventId,
    eventName,
    child,
    onRemoved,
    ...
    const handleRemove = async () => {
        try {
            setIsRemoving(true);
            setError(null);
            await makeApiRequest(`/event/${eventId}/attendees/${child.id}`, {
                method: 'DELETE',
            });
            await onRemoved();
            closeModal();
    ...
    return (
        ...
        >
            <div className="mb-4 flex items-center">
                ...
                <h2 id="remove-attendee-title" className="text-xl font-bold text-gray-800">
                    Remove attendee
                </h2>
            </div>

            <p className="mb-2 text-gray-700">
                Remove <strong>{childName}</strong> from <strong>{eventName}</strong>?
            </p>
