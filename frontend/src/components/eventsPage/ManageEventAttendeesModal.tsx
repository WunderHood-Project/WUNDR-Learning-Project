...
export default function ManageEventAttendeesModal({ event, onParticipantCountChange }: Props) {
  ...
  const loadAttendees = async () => {
    ...
    try {
      setLoading(true);
      setLoadError(null);
      const response = await makeApiRequest<AttendeesResponse>(`/event/${event.id}/attendees`);
      setAttendees(response.children ?? []);
      onParticipantCountChange(event.id, response.participants);
    } catch (error) {
  ...
  useEffect(() => {
    void loadAttendees();
    // The modal represents one event for its lifetime.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [event.id]);

  const removeAttendee = async () => {
    if (!event.id || !selectedChild) return;
    try {
      setRemovingChildId(selectedChild.id);
      setRemoveError(null);
      await makeApiRequest(`/event/${event.id}/attendees/${selectedChild.id}`, { method: 'DELETE' });
  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="manage-attendees-title" className="mx-auto w-[min(92vw,36rem)] rounded-2xl bg-white p-5 shadow-lg sm:p-6">
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          <h2 id="manage-attendees-title" className="text-xl font-bold text-wondergreen">Manage attendees</h2>
      ...
      {loading ? (
        ...
        <p className="rounded-xl bg-wonderbg p-5 text-center text-sm text-gray-700">No children are enrolled in this event.</p>
      ) : (
      ...
      {selectedChild && (
        <div className="mt-5 rounded-xl border border-red-200 bg-red-50 p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-gray-800">Remove <strong>{selectedChild.firstName} {selectedChild.lastName}</strong> from <strong>{event.name}</strong>?</p>
